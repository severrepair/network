...
public class ChatServer4 {
	...
	ServerSocket server;
	int port = 8005;
	ChatMgr4 mgr; //로그인 관련객체
	MsgMgr4 mgr2;	//메세지 관련객체
	...
	class ClientThread4 extends Thread {
		...
		public void routine(String line) {
			...
			if (cmd.equals(ChatProtocol4.ID)) {
				...
				data/* 오늘 머해 */ = data.substring(idx + 1);//msg:data
				ClientThread4 ct=findClient(cmd);
				if(ct!=null) {
					MessageBean4 bean = new MessageBean4();
					bean.setFid(id);//aaa
					bean.setTid(cmd);//bbb
					bean.setMsg(data);//오늘머해
					mgr2.insertMsg(bean);//DB저장
					ct.sendMessage(ChatProtocol4.MESSAGE + ChatProtocol4.DM1+
				...
				Vector<MessageBean4> vlist=mgr2.getMsgList(id);
				String str = "";
