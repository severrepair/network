...
implements ActionListener, Runnable {
	...
	public void run() {//서버로부터 들어옴 (multi thread)
		try {
	...
	public void routine(String line) {//서버로부터 들어옴 (multi thread)
	...
	public void actionPerformed(ActionEvent e) {// 이벤트 발생(동작) (to server)
		...
		if(obj==bt1) {
			...
			if(filterMgr(str)) {
				new MDialog(this,"경고","입력하신 글자는 금지어입니다.");//모달
				return;
			}
			//id입력 경우 또는 채팅
			if(!flag) {//id입력
